...
**Stage 7 (delivery, packaging and submission readiness) is implemented and locally validated on `build/final-delivery`; it is not merged yet.** It adds no product functionality. It adds reviewer reproducibility and delivery evidence: a current `.env.example` contract, an idempotent demo seed with tracked fixtures, Swagger UI plus a committed OpenAPI document and a drift check, liveness and readiness endpoints, non-root multi-stage images and a Compose stack with a one-shot migration service, a tracked-only clean-checkout rehearsal, security scanning with triage, and the reviewer README and architecture memo. No Prisma schema change and no migration were needed.
...
| `pnpm test:smoke` | The packaged reviewer journey against a running Compose stack |
| `pnpm openapi:generate` / `pnpm openapi:check` | Rewrite the committed `docs/openapi.json`, or fail when it is stale |
| `docker compose up --build -d` + `docker compose run --rm seed` | The documented reviewer path: migrate, start and seed the packaged stack |
...
Stage 7 added the packaging and delivery surface that earlier milestone notes listed as proposals: `Dockerfile`s and `docker-compose.yml` exist, `pnpm openapi:generate` / `pnpm openapi:check` manage the committed OpenAPI document, and `pnpm test:smoke` drives the packaged stack. `test:unit` and `security:check` remain proposals — the suites and scans that exist are named exactly as they are invoked.

## Test evidence

- Test suites today: `apps/api/test/auth.e2e-spec.ts`, `apps/api/test/products.e2e-spec.ts`, `apps/api/test/assets.e2e-spec.ts`, `apps/api/test/product-attachments.e2e-spec.ts`, `apps/api/test/publication.e2e-spec.ts`, `apps/api/test/public-passport.e2e-spec.ts`, `apps/api/test/passports.e2e-spec.ts`, `apps/api/test/passport-pdf.e2e-spec.ts`, `apps/api/test/passport-pdf-stream.spec.ts`, `apps/api/test/analytics.e2e-spec.ts`, `apps/api/test/passport-cache.e2e-spec.ts`, `apps/api/test/passport-content-cache.spec.ts`, `apps/api/test/lifecycle.e2e-spec.ts`, `apps/api/test/users.e2e-spec.ts` and `apps/api/test/settings.e2e-spec.ts` via `pnpm --filter @notarify/api test:integration` (15 suites, 213 tests: fourteen integration suites against a real PostgreSQL plus the database-less cache-port spec; the cache suite additionally needs a real Redis, and `REDIS_URL` selects it); `e2e/auth.spec.ts`, `e2e/products.spec.ts`, `e2e/assets.spec.ts`, `e2e/public-passport.spec.ts`, `e2e/passport-ui.spec.ts`, `e2e/passports.spec.ts`, `e2e/passport-pdf.spec.ts`, `e2e/analytics.spec.ts`, `e2e/lifecycle-admin.spec.ts` and `e2e/stage4-acceptance.spec.ts` via `pnpm test:e2e` (71 Playwright tests, built API + web; `e2e/release-smoke.spec.ts` adds one packaged-stack journey via `pnpm test:smoke`). The Stage 4 evidence map is [docs/STAGE4-ACCEPTANCE.md](docs/STAGE4-ACCEPTANCE.md). Never report a test, scan, or audit as passing unless you ran it and can quote the command and its result.
