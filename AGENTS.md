...
- **`apps/api`** — NestJS 12.0.4 family, Prisma through `@prisma/adapter-pg`.
  ...
  - Cache: `Redis` holds only the interpreted content of an already-selected immutable published version, keyed by passport, version and content schema, bound by digest to the snapshot it came from and bounded by a TTL. `REDIS_URL` is optional; an absent, unreachable or corrupt cache falls back to PostgreSQL.
...
Not implemented, and not to be assumed: product delete/withdraw, public historical-version routes, Users/Settings flows, tenancy onboarding, garbage collection, antivirus or PDF CDR, object storage, Docker/Compose and deployment. Analytics has **no** automatic raw-retention or purge job in this assessment: accepted events stay in `AnalyticsEvent`, and production retention is future hardening. Product delete plus a read-only `View` destination remain Stage 6 work; `Total Views` is now a measured value rather than a placeholder.
...
- Redis caches **only** the interpreted content of an immutable published version, keyed by passport, version and content schema, bound by digest to the exact snapshot it was derived from and bounded by a TTL. Nothing else is cached: not visibility, not the current-version pointer, not sessions, analytics, QR redirects, asset bytes or generated PDF bytes.
...
- Test suites today: `apps/api/test/auth.e2e-spec.ts`, `apps/api/test/products.e2e-spec.ts`, `apps/api/test/assets.e2e-spec.ts`, `apps/api/test/product-attachments.e2e-spec.ts`, `apps/api/test/publication.e2e-spec.ts`, `apps/api/test/public-passport.e2e-spec.ts`, `apps/api/test/passports.e2e-spec.ts`, `apps/api/test/passport-pdf.e2e-spec.ts`, `apps/api/test/passport-pdf-stream.spec.ts`, `apps/api/test/analytics.e2e-spec.ts`, `apps/api/test/passport-cache.e2e-spec.ts` and `apps/api/test/passport-content-cache.spec.ts` via `pnpm --filter @notarify/api test:integration` (12 suites, 182 tests, real PostgreSQL; the cache suite additionally needs a real Redis, and `REDIS_URL` selects it); `e2e/auth.spec.ts`, `e2e/products.spec.ts`, `e2e/assets.spec.ts`, `e2e/public-passport.spec.ts`, `e2e/passport-ui.spec.ts`, `e2e/passports.spec.ts`, `e2e/passport-pdf.spec.ts`, `e2e/analytics.spec.ts` and `e2e/stage4-acceptance.spec.ts` via `pnpm test:e2e` (59 Playwright tests, built API + web). The Stage 4 evidence map is [docs/STAGE4-ACCEPTANCE.md](docs/STAGE4-ACCEPTANCE.md). Never report a test, scan, or audit as passing unless you ran it and can quote the command and its result.
