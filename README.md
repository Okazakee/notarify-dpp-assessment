...
> **Status: Stages 0–3 complete on `main`.** The validated schema and initial PostgreSQL migration, the hardened authentication flow, Product draft CRUD with optimistic concurrency, and the Assets slice — validated upload with private retrieval, plus image, document and certification-PDF attachments — are implemented, verified and merged into `main`. Stage 4 (publication and Product Passports) is next, delivered as six gated milestones. Publication, passports, QR, PDF export, analytics, Redis, dashboards, Users/Settings, product delete and deployment are not implemented. Nothing here is a claim of working software beyond what the sections below describe.
...
Not implemented: product delete/withdraw and soft delete, publication and Product Passports, QR, PDF export, analytics, Redis, dashboards, Users/Settings, and deployment. All nine assessment bonuses remain in scope; see the [roadmap](docs/specs/00-ROADMAP.md) for where each one lands.
