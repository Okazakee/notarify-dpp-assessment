...
**Date:** 2026-09-21  
**Status:** the schema was validated and migrated to PostgreSQL 18.6, and the application has since moved well past this record — authentication, Product draft CRUD and the Assets slice are implemented. Read the decision and enforcement sections as live contracts, and the schema-validation section as the state at that round. The project-wide, current state lives in [AGENTS.md](../AGENTS.md).  
...
| Publish authorization | Editors may publish and republish; publishing is not Admin-only. Admin additionally deletes/withdraws, reviews versions, reads raw analytics and audits, and manages users and settings. | Supersedes the earlier Admin-only proposal. The brief requires neither behaviour, so this is a project choice, and it **is** recorded — it is not pending. Publication is now implemented, and the endpoint is deliberately role-agnostic: it requires an authenticated actor and allows EDITOR and ADMIN alike, so nothing gates publish to ADMIN. |
...
Approved by Cristian in the milestone prompt for the Assets slice, implemented on `build/assets` and merged into `main`. Recorded here rather than by rewriting the planning text in the specs, which stays as it was written.
...
### B6. Roadmap normalization and assessment semantics

Recorded 2026-09-23. The roadmap was revalidated against the original assessment and normalized into smaller gated delivery slices while retaining all required functionality and all nine listed bonuses. The current delivery plan distinguishes assessment requirements, explicit bonuses and implementation details so that each milestone has a bounded acceptance surface. `docs/specs/00-ROADMAP.md` owns the stage sequence; this section records what changed and the two semantics that settled with it.
...
| Brand logo semantics | The public Passport displays a brand logo, satisfied by a bundled application brand asset. Publication does **not** require `Company.logoAssetId`, and Settings/company-logo upload is not a publication prerequisite. The schema support stays and may be used later if Settings work makes it worthwhile. |
| Users/Settings placement | Stage 6, as required work proportional to the brief. Stage 2 delivered the authentication and authorization foundation only. |
...
| Prerequisites | Publication-only, never enforced at draft save: the seven client-settable basic fields; sustainability present **with all five fields** non-null (carbon footprint, water consumption, recycled material percentage, repairability score, recyclable); a cover image; materials totalling 100 when any are present; each certification complete with name, issuing authority, issue date, **expiration date** and PDF, and a non-empty name and authority; a production date that is not in the future (date-only comparison); and every referenced asset revalidated. Failures return 400 `PUBLICATION_INCOMPLETE` naming the missing fields. The company logo is **not** a prerequisite. |
| Asset revalidation | Every asset the version will reference is re-proven inside the publish transaction and **before** version creation to be `ACCEPTED`, owned by the publishing company, and of the required MIME family. A failure leaves no Passport on first publication, no new version on republish, an unchanged `currentVersionId` and no retained references. The error is identical for missing, foreign-company and non-accepted assets, so it cannot be used to probe for another company's assets. |
| Brand in the snapshot | Stage 4.1 stores the company display name only. `logoAssetId` is **not** snapshotted and no `COMPANY_LOGO` retained reference is written; `Company.logoAssetId` stays unused infrastructure, and the eventual public page may use a bundled brand asset. |
| Audit | Publication writes **no** `AuditEvent`. The audit-log bonus is a separate milestone with its own event and action policy, so 4.1 does not begin it with one isolated event type. |
...
| Atomicity | The version, its `PassportVersionAsset` rows, the QR artifact and the current pointer are written in one transaction, so a passport is never observable half-published. Publication writes **no** `AuditEvent`: the audit-log bonus is a separate milestone with its own event and action policy. |
...
| Caching | `no-store` on every public response. No Redis, no application cache, no ISR: caching is revisited in the Redis milestone once withdrawal and republish semantics are in place. Stage 5 later added a server-side cache of immutable content only, and the public response policy is unchanged — see B12. |
| Analytics | None at this milestone. No `AnalyticsEvent` or `AnalyticsDaily` write, and no IP or user-agent parsing. Superseded by Stage 5 — see B12. |
...
| Shared presentation | One `PassportPresentation` component with one explicit display model and two adapters: the public `PassportView` projection and the editor draft. The model holds no fetching, no editor chrome and no fabricated publication records. The draft adapter supplies eventual public presentation state; the unpublished indication stays in editor chrome. |
| Preview identity | Values that cannot exist before publication are explicit placeholders, never fabricated identifiers or next version numbers. The shared presentation simulates eventual `PUBLISHED` / `VERIFIED` state, including the qualified prototype badge; editor-only chrome identifies the content as unpublished. The real public UUID, version and public URL appear only when known from an actual publication in this editor session. Preview performs no publication write. |
...
### B10. Published-edit and historical visibility (Cristian decision, 2026-09-24)

Cristian explicitly settled published-edit visibility: editing mutable Product draft data never changes the current public Passport. Unsaved and saved draft changes remain private until an explicit successful republish creates a new immutable `PassportVersion` and moves the Passport's current-version pointer. Preview shows the current draft being considered for publication; `/passport/:uuid` reads only the current published version. The public UUID and QR remain stable across republish. This is a human decision, not an AI-selected default.
...
| Caching | None at this milestone: no PDF cache. A republish changes what the same URL exports immediately. Stage 5 later added a cache for the shared immutable content path only; a republished export is still never stale, because the current version is resolved from PostgreSQL first — see B12. |
...
### B12. Analytics, dashboard and Redis cache decisions (Stage 5, Cristian decision 2026-09-25)

Cristian settled these by supplying the Stage 5 instruction; they were previously open proposals. The recorded decisions are:

| Decision | Recorded outcome |
| --- | --- |
| `QR_HIT` meaning | An accepted server request to the stable resolver `/q/:uuid`. It is not proof of a physical camera scan, a unique human or a unique device, and it is never summed with `VIEW` into a "visitors" figure. |
| `VIEW` meaning | One visible public Passport page navigation reported by the page's own client tracker. The editor Preview, the historical version view, the JSON projection, asset downloads, QR downloads and PDF downloads never emit one. |
| `VIEW` idempotency | The client generates one UUID `eventKey` per navigation and reuses it on retry. The unique constraint plus conflict-safe insertion make a duplicate a no-op; the daily bucket only moves in the transaction that inserted the raw row. |
| Event transaction | One transaction writes the raw `AnalyticsEvent` and increments/upserts its `AnalyticsDaily` UTC bucket, so the aggregate can never drift from the raw rows. |
| Server authority | The `VIEW` body carries only `eventKey` and the public version number; unknown properties are rejected. Time, address, browser, operating system, language, country, `source` and `synthetic` are server-resolved. A public client can never set `synthetic`. |
| Synthetic events | Runtime events always record `synthetic = false`, and every real metric excludes `synthetic = true`. |
| UTC reporting | Reporting is UTC. `scansToday` is the current UTC calendar day; the weekly series is exactly seven zero-filled UTC buckets, oldest first. No local-time or DST analytics. |
| Retention | The previously proposed 7-day raw / 90-day daily retention job is **not** implemented in this assessment. There is no scheduler, cron worker or purge subsystem; accepted events remain in `AnalyticsEvent`, `AnalyticsDaily` is maintained at ingestion, and production retention is documented as future hardening. |
| Mock country | `ANALYTICS_MOCK_COUNTRY` (default `IT`) records the country with `countrySource = MOCK`, and the UI labels it as mocked. Country is never inferred from IP, language or browser locale. |
| Roles | Both roles may read `GET /dashboard` and `GET /analytics`, company-scoped. Only the Admin projection carries the raw IP address on recent scans; the Editor response omits the property entirely. Response shaping is authoritative, not CSS hiding. |
| Raw IP privacy | The address is stored because the assessment asks for it, is Admin-only, never appears in a public response, is never treated as proof of identity and is never called anonymized. There is no automatic retention, recorded as a prototype limitation. |
| Redis role | Redis is a **disposable cache only**. PostgreSQL owns visibility, the current-version pointer, publication truth, withdrawal and deletion truth, analytics and sessions. A Redis outage never makes a valid Passport unavailable. |
| Redis scope | Only the interpreted content of an already-selected immutable published version. Not cached: visibility, the current-version pointer, auth or session data, analytics, QR redirects, asset bytes, historical authenticated responses or generated PDF bytes. |
| Redis key | `notarify:passport:{passportId}:version:{versionId}:schema:{snapshotSchemaVersion}`. It carries the immutable identity, so a republish selects a different key by construction; the public UUID alone would be wrong because it deliberately survives a republish. |
| Redis integrity | A cached entry carries an unkeyed SHA-256 checksum over a domain separator, the serialized snapshot PostgreSQL selected and the serialized interpreted content. Both inputs are covered, because a snapshot-only digest leaves the content itself unverified and would let a shape-valid but modified or transplanted payload replace published content. An entry is discarded — never served — if it is unreadable, oversized, written by another schema, its content no longer matches the checksum, or it does not belong to the snapshot PostgreSQL selected. The checksum binds content to that snapshot but does not encode the storage key, so an entry transplanted to another version's key passes only if that version's snapshot and content are byte-identical, which serves the same content rather than something wrong. The checksum detects corruption and inconsistency; it is **not** authentication against an actor who controls Redis and can recompute it, which is out of scope for this assessment. |
| Redis TTL | Bounded, default 300 seconds via `REDIS_CACHE_TTL_SECONDS` (validated 1–86400). Expiry is hygiene; correctness never depends on it. |
| Redis optionality | `REDIS_URL` may be absent, in which case caching is disabled and PostgreSQL behaviour is fully functional. A configured but unreachable cache falls back within bounded connect and **socket** timeouts — the socket inactivity timeout is what bounds a connected-but-silent server, because the client library drops the per-command timeout once a command is written — with `disableOfflineQueue` so no unbounded offline queue exists. Startup does not fail because Redis is down. |
| Redis client and server | `@redis/client` 6.2.1 (MIT, Node ≥20, built-in types) rather than the full Redis Stack client, because only basic string commands are needed. Redis 8.10.2 (OSS) is the verified server for CI and local evidence. No RedisJSON/Search/Bloom/TimeSeries modules. |
| Ingestion rate bound | A bounded per-process limiter, deliberately **not** Redis-backed so a cache outage cannot disable it and it cannot fail a valid QR redirect. It is a technical bound (30 accepted events per minute per address and passport), not a business metric, and not a distributed limiter. |
| Dashboard counters | Active-only by policy: `totalProducts` counts non-deleted products, `publishedPassports` counts active passports with a current version, `generatedQrCodes` counts active passports with the generated artifact (equal by construction under the current schema), and `totalPassportViews` sums non-synthetic `VIEW` counts for active passports. A counter may therefore decrease after a future withdrawal; monotonic semantics are not invented. |
| Product `Total Views` | Non-synthetic `VIEW` count for the product's currently active Passport. A QR scan never contributes, an unpublished product reports a measured `0`, and the value is read in one bounded aggregate per page rather than one query per row. |
| Analytics scope boundary | No `AuditEvent` is written by any Stage 5 path; the audit-log bonus was delivered separately in Stage 6 — see B13. No queue, worker, scheduler, event bus or telemetry framework was introduced. |
...
| History lifecycle truthfulness | Immutable publication history and current public availability are separate facts. `GET /passports/:passportId/versions` reports `lifecycleStatus` from PostgreSQL state and leaves `publicUrl`/`qrDownloadUrl`/`pdfDownloadUrl` null while withdrawn, because those endpoints return the uniform 404; the history UI states the withdrawal and omits the public actions rather than rendering dead links. A withdrawn Passport keeps its UUID, retained versions, QR bytes and current-version pointer, and the UI calls that pointer the last published version rather than the publicly served one. |
...
| Seed policy | `pnpm db:seed` creates no duplicates and is scoped to the demo fixtures: deterministic ids mean a second run adds nothing and reviewer-created products, users and assets are never touched. The seeded fixtures themselves are reset to their deterministic state — nested content, scalar fields, company display name and freshly salted password hashes — and a withdrawn demo Passport is restored, so re-seeding recovers the demo without destroying the volume. |
...
| Missing lifecycle boundary | Draft save is allowed before every publication field is complete. | 03 §Constraints and lifecycle; 05 §Product editing contract | Keep draft product/sustainability/certification fields nullable where incomplete entry is meaningful. Publish performs stronger transactional completeness, ranges, date, ownership, cover and material-total checks. |
...
| Semantics gap | “Verification” could be mistaken for product authenticity. | 01 §Mock verification; 04 §Access model; 06 §Publication and verification | **Settled 2026-09-23:** the badge is a prototype/application-level indicator on an active published version. No review workflow is required and `PassportReview` may stay unused; if one is ever built, review belongs to exactly one version, absence means unreviewed, new versions never inherit it, and changes are audited. Public wording stays qualified. |
...
`Company` owns users, products, and assets. `Product` is a nullable/incomplete live draft with normalized materials, optional sustainability, certifications, images, and documents. Assets carry metadata and are one-to-one with bytea content. Each product can have one `Passport`; it has a stable public UUID and current version pointer. Each immutable `PassportVersion` captures JSONB public projection and retains the exposed product Assets through `PassportVersionAsset`; the brand mark is bundled by the web application, not retained as a Company logo Asset. A version can have one separately mutable `PassportReview`, but no review subsystem or review audit writes are implemented. Sessions own refresh-token chains; analytics reference passports and optionally their captured version. Audit records retain mutation evidence; `actorId` is nullable only for system/seeded actions with no authenticated human actor or a deliberately retained record after a permitted actor hard-delete.
...
| Publication prerequisites and idempotency | expected draft revision, required basic fields, all five sustainability fields, cover image, materials totalling 100, complete certifications, non-future production date, revalidated accepted same-company assets, Passport/version rows | Transactional rule: `publication.publish_expected_revision_tx` | Lock/conditionally claim the product; build one coherent snapshot, asset-reference set, QR state and current pointer or expose none. |
...
Publish accepts `expectedDraftRevision` and locks or conditionally claims that revision. A concurrent edit cannot produce a mixed snapshot: expected revision publishes atomically or caller gets a conflict/retries. First publish creates one Passport and one stable UUID; `Passport.productId` unique prevents a second passport. `(passportId, sourceDraftRevision)` makes same-revision replay idempotent and `(passportId, versionNumber)` prevents duplicate numbering. Snapshot, retained asset links, QR state and current pointer appear in one transaction; failed publication exposes none. No audit row is written by publication. A successful replay returns the existing version without incrementing. Later draft edits remain private until republish. Review state is not part of publication: no review workflow is required, so a new version carries no review state rather than being described as unreviewed.

### 5. Asset retention, authorization, and soft deletion

Uploaded content is immutable; replacement creates a new Asset. Every snapshot has relational retention links for every file it exposes; the company logo is **not** snapshotted and writes no `COMPANY_LOGO` retained reference (see section B7). `PassportVersionAsset` and `AssetContent` use non-cascading restrictive semantics. Soft deletion never cascades into historical records. Anonymous download eligibility is recomputed from active, non-deleted product/passport/current version and retained version-asset reference; asset ID knowledge, stale snapshot JSON, a hypothetical asset public flag, or Redis cannot authorize it. Historical assets are back-office-only (settled in B10). Garbage collection may remove only grace-period-expired assets that have no live draft, logo, certification/document/image, or historical-version reference. Cache invalidation is hygiene, never correctness.
...
| 4. Publish/replay | Lock product/revision; create/reuse Passport; create version/asset refs; update QR/current pointer (no `AuditEvent`; audit-log bonus is separate) | one Passport/product; `(passportId, sourceDraftRevision)`; `(passportId, versionNumber)`; restrictive historical FKs | Roll back every publication write; no mixed snapshot | Same successful revision returns existing version; stale edit/publish gets conflict | Versions and version-linked assets retained after withdrawal/deletion. |
