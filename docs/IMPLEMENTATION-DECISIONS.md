...
| Redis integrity | A cached entry is bound by a SHA-256 digest to the exact snapshot it was derived from, bounded in size before parsing, and discarded if it is unreadable, oversized, written by another schema or bound to a different snapshot. |
