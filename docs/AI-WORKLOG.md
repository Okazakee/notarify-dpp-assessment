...
**Result.** Stage 7 is implemented and locally validated on `build/final-delivery`; it awaits Cristian's decision and has not been merged. No tag has been created. Stage 7 added no product functionality and no schema change, and the application's behaviour is unchanged apart from the web-origin security headers and the QR bridge implementation.
