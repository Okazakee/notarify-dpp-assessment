...
**AI participation.** Pi inspected the adapter, shared presentation, browser tests, schema and owning documents; made the narrow corrections and ran local tests. A separate read-only `review` subagent is used for the final branch diff. No external AI model participated in this round.
