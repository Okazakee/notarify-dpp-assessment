...
**Update 2026-09-26 (Stage 7).** Delivery and packaging are implemented. The repository now carries multi-stage, non-root `Dockerfile`s for the API and web, a `docker-compose.yml` reviewer stack (private PostgreSQL 18.6 and Redis 8.10.2, a one-shot migration service the API waits on, an explicit seed service behind a profile, health checks throughout and one durable volume), a committed `docs/openapi.json` with `pnpm openapi:generate` / `pnpm openapi:check`, API liveness and readiness plus a web health route, and `pnpm test:smoke` driving the packaged stack. CI gained an OpenAPI drift check and a second job that builds and exercises the Compose stack. Security scanning (dependency audit, Gitleaks, Trivy on the repository and the images, and a ZAP baseline) was run and triaged; the results and their classification are in `docs/AI-WORKLOG.md`.

**Update 2026-09-25 (Stage 6).** The integration contract grew to **15 suites / 213 tests** with `apps/api/test/lifecycle.e2e-spec.ts` (transactional audit, soft delete and withdrawal, delete-versus-save and delete-versus-publish races, warm-cache deletion, retained history and analytics, authorization and company isolation), `apps/api/test/users.e2e-spec.ts` (list/create/patch, role change observed by an existing token, deactivation revoking sessions, reactivation, last-active-Admin protection including a concurrent race, and isolation) and `apps/api/test/settings.e2e-spec.ts` (display name, logo ownership and type rules, and publication-snapshot isolation).

`pnpm test:e2e` grew to **71 Playwright tests** with `e2e/lifecycle-admin.spec.ts` (final Product actions, delete confirmation and cancellation, the read-only view versus the public page, six-item Admin navigation versus four-item Editor navigation, user administration including the last-Admin refusal, and settings with audit activity).

**Update 2026-09-25 (Stage 5).** The integration contract grew with the addition of `apps/api/test/analytics.e2e-spec.ts` (ingestion, `VIEW` idempotency including concurrent retries, UTC boundaries, dashboard counters, ranking ranges, role-shaped scan projections and Product totals), `apps/api/test/passport-cache.e2e-spec.ts` (hit/miss, TTL, corrupt and wrong-version payloads, a dead endpoint, republish key switching and withdrawn/deleted passports behind a warm entry) and `apps/api/test/passport-content-cache.spec.ts` (the disabled and enabled configurations of the cache port itself, and the version-specific key shape).
...
The absent-`REDIS_URL` configuration is proven at the port level rather than by reconfiguring a running application: the validated environment is loaded once per process, so an integration test cannot remove the variable from an application that already started. `passport-content-cache.spec.ts` asserts that an unconfigured port reports itself disabled and turns every operation into a harmless miss, and the integration suite proves the unreachable-cache fallback against a real server.
