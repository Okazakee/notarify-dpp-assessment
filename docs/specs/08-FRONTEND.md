...
**Update 2026-09-25 (Stage 6):** the remaining back office is implemented. One shared navigation component exposes all six Admin destinations (Dashboard, Products, Product Passports, Analytics, Users, Settings) and the first four to an Editor. `/users` provides a bounded list with create, role change and enable/disable behind confirmation; `/settings` manages the company display name and logo and shows Recent Audit Activity; `/products/:id/view` renders the current private draft read-only with an explicit Edit Product action; and the Product table adds View for both roles and Delete for an Admin behind a dialog that states the withdrawal and retention semantics.
