...
3. Publish takes an expected revision. Within a database transaction, lock/conditionally claim that product revision, verify all publish rules, create or reuse its Passport, and create the immutable version plus its retained asset references. **Update 2026-09-25 (Stage 6):** publication now writes exactly one `PASSPORT_VERSION_PUBLISHED` audit row inside the same transaction as the new immutable version. A replay of an already-published revision writes none, because no new version was created. Publication still audits nothing else.
...
7. **Implemented in Stage 6:** `DELETE /products/:id` is Admin-only and soft-deletes the product, withdraws its passport and records a `PRODUCT_DELETED` audit event in one transaction, keeping every version and file. Public content and downloads stop immediately. The stable URL deliberately returns the **uniform safe 404** rather than a `410 Gone` tombstone: the accepted public contract across Stages 4–5 is one identical not-found body, and Stage 6 preserves it. This is a demo policy, not a claim of legal retention compliance.
