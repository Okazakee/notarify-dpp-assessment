...
**Update 2026-09-25 (Stage 6):** the lifecycle now includes withdrawal, so the history response carries an explicit lifecycle state. `GET /passports/:passportId/versions` returns a `PassportHistorySummary` with `lifecycleStatus: 'ACTIVE' | 'WITHDRAWN'` derived from `Passport.withdrawnAt` and `Product.deletedAt`, and its `publicUrl`, `qrDownloadUrl` and `pdfDownloadUrl` are **null while withdrawn** rather than mechanically constructed, because every one of those public endpoints returns the uniform 404 after withdrawal. The normal `GET /passports` list stays active-only and keeps its active-only row shape. The history UI states the withdrawn lifecycle and omits the public actions instead of offering dead links. Withdrawal is operational Passport state, not historical content: a retained version still reports that it was published, and no public route, cache rule or asset authorization changed.
...
**Update 2026-09-25 (Stage 4.5):** the PDF export is implemented as `GET /passport/:uuid/pdf` on the anonymous public surface, using the same active-visibility resolution as `GET /passport/:uuid`, so malformed, unknown, withdrawn and soft-deleted states all return the same safe JSON 404 and a historical version has no PDF route. The document is rendered server-side with `pdfkit` 0.20.2 (MIT) from the current immutable `PassportVersion.publicSnapshot` through the shared projection; it embeds the **stored** `Passport.qrPngBytes` and never regenerates a code, and every embedded image must be retained by the current version and still be an accepted asset with stored content. WebP is converted to PNG in memory with the existing `sharp` dependency and dimensions are bounded to 1600 px without upscaling, while JPEG and PNG are embedded unchanged when they are already small enough. Text uses embedded full Noto Sans regular/bold (`@expo-google-fonts/noto-sans` 0.4.2, font files OFL-1.1) rather than PDF Standard 14 fonts, so mixed European text such as `Caffè Torino · München · Łódź` survives. The response streams the PDFKit document to the client, sets `Content-Type: application/pdf`, an `attachment` disposition with the deterministic `notarify-passport-{uuid}-v{version}.pdf` filename, `nosniff`, `no-store` and a `cross-origin` resource policy, and aborts the response if rendering fails after streaming begins. There is no PDF cache: a republish changes what the same URL exports immediately. The Stage 5 content cache cannot make an export stale, because the current version is always resolved from PostgreSQL before cached content is considered. The public page and Product Passports expose the action; draft Preview and historical versions do not. `pdfjs-dist` is a test-only devDependency used to inspect generated output independently of PDFKit.
