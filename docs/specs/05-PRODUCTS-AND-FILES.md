...
**Update 2026-09-25:** Stage 4.4 implemented the back-office version history. The product list response now also returns `coverImageAssetId` (the current draft cover; bytes remain behind the authenticated asset route) and a nullable `passport` summary (`publicUuid`, `publicUrl`, `qrDownloadUrl`, `currentVersionNumber`, `sourceDraftRevision`, `hasUnpublishedChanges`, `currentPublishedAt`), fetched with the bounded list query rather than one request per row. The Product table renders a cover image, a QR action and Open Passport/Download QR actions for published rows. **Update 2026-09-25 (Stage 6):** Product deletion and the read-only `View` destination are implemented. `DELETE /products/:id` is Admin-only and soft-deletes the product while withdrawing its published Passport in one transaction, and `/products/:id/view` renders the current private draft read-only. The Product table now offers View, Edit, Delete (Admin only), Open Passport and Download QR.

**Update 2026-09-25 (Stage 5):** `totalViews` is now part of the bounded product-list response and of `ProductDetail`, computed as the non-synthetic `VIEW` count for the product's currently active Passport. It is read in one aggregate over the page's passports, so the column never becomes a query per row; an unpublished product and a published product with no views both report a real `0`, and a QR scan does not contribute. A mutation response reads its own view total inside the same transaction as the write, so a committed save and the detail it returns cannot disagree. The Product table renders the measured number instead of the Stage 4.4 placeholder.

**Update 2026-09-25 (Stage 4.5):** the earlier statement that the public page, version history and PDF export "remain later Stage 4 milestones" is superseded. The public page and the back-office version history shipped in Stages 4.3–4.4, and the Passport PDF export of the current published version shipped in Stage 4.5 as `GET /passport/:uuid/pdf`. Stage 4.6 (the Stage 4 acceptance suite) is the remaining Stage 4 milestone; the Product table gaps above are unchanged.
